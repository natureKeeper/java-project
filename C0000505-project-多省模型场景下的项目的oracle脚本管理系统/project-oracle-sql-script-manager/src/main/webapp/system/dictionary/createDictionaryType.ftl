...
<td class="x_banner_bar" ondblclick="//switchView();">
 &nbsp; <font class="x_banner_font">新增 DictionaryType</font>
</td>
</tr>
...
<td>
	...
	<td valign="top" style="padding:10;">	
		<div clas="box">
			...
			<input id="dictionaryType.metaDictionaryTypeId" name="dictionaryType.metaDictionaryTypeId" value="${metaDictionaryTypeId}" type="hidden" />
			<input id="dictionaryType.enable" name="dictionaryType.enable" value="true" type="hidden" />
			<tr>
			<td class="x_form_label" style="width:25%;">name 名称</td>
			<td class="x_form_field"><input id="dictionaryType.name" name="dictionaryType.name" value="${(dictionaryType.name)?if_exists}" class="x_text" /></td>
			</tr>
			<tr>
			<td class="x_form_label">code 编码</td>
			<td class="x_form_field"><input id="dictionaryType.code" name="dictionaryType.code" value="${(dictionaryType.code)?if_exists}" class="x_text" /></td>
			</tr>
			</form>
			</table>
		</div>		
...
function doCreate() {
	...
	J.json(
		'systemDictionaryManagement!doCreateDictionaryType.html',
		p.join('&'),
		function(o) {
			Form.disable('fom');
