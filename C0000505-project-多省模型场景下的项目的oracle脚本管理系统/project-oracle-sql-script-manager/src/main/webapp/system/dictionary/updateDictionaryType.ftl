...
<td class="x_banner_bar" ondblclick="//switchView();">
 &nbsp; <font class="x_banner_font">修改 DictionaryType</font>
</td>
</tr>
...
<td>
	...
	<td valign="top" style="padding:10;">	
		<div clas="box">
			...
			<input id="dictionaryType.id" name="dictionaryType.id" value="${(metaDictionaryType.id)?if_exists}" type="hidden" />
			<input id="dictionaryType.version" name="dictionaryType.version" value="${(metaDictionaryType.version)?if_exists}" type="hidden" />
			<tr>
			<td class="x_form_label" style="width:25%;">name 名称</td>
			<td class="x_form_field"><input id="dictionaryType.name" name="dictionaryType.name" value="${(metaDictionaryType.name)?if_exists}" class="x_text" onkeyup="//onTextKeyUp();" /></td>
			</tr>
			<tr>
			<td class="x_form_label" style="width:25%;">code 编码</td>
			<td class="x_form_field"><input id="dictionaryType.code" name="dictionaryType.code" value="${(metaDictionaryType.code)?if_exists}" class="x_text" onkeyup="//onTextKeyUp();" /></td>
			</tr>
			</form>
			</table>
		</div>
...
function onNameKeyUp() {
	//$('dictionaryType.code').value = $('dictionaryType.entityName').value + '.' + $('dictionaryType.attributeName').value
}
function onTextKeyUp() {
	//$('dictionaryType.name').value = $('dictionaryType.entityText').value + '.' + $('dictionaryType.attributeText').value
}
function doUpdate() {
	var p = [];
	...
	J.json(
		'systemDictionaryManagement!doUpdateDictionaryType.html',
		p.join('&'),
		function(o) {
			Form.disable('fom');
