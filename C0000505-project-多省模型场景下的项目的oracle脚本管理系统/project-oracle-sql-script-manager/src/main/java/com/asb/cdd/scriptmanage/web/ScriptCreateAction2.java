...
public class ScriptCreateAction2 extends AbstractAction {

	private static final transient Log log = LogFactory.getLog(ScriptCreateAction2.class);
	...
	public int getScriptSeq() {
		return scriptSeq;
	}

	public void setScriptSeq(int scriptSeq) {
	...
	public String submitFormCheck(){
		//打开提交页面加载时判断
		if(files.size()==0 || scripts.size()==0 || fileId.equals("") || (files.size()<Integer.parseInt(fileId))){
			return " ";
		}
		
		List<ScriptTable> newScripts = new ArrayList<ScriptTable>();
		newScripts.add(scripts.get(Integer.parseInt(fileId)-1));
		scripts = newScripts;
		//截取上传文件，检测特定文件
		List<File> newFiles=new ArrayList<File>();
		List<String> newFilesFileName=new ArrayList<String>();
		List<String> newFilesContentType=new ArrayList<String>();
		newFiles.add(files.get(Integer.parseInt(fileId)-1));
		newFilesFileName.add(filesFileName.get(Integer.parseInt(fileId)-1));
		newFilesContentType.add(filesContentType.get(Integer.parseInt(fileId)-1));
		files = newFiles;
		filesFileName = newFilesFileName;
		filesContentType = newFilesContentType;
		...
		try {
			...
			//e.printStackTrace();
			msg = "校验输入异常: " + e.getMessage().trim();
		}
		
		if(msg!=""){
			sb.append("<font color='blue'>"+msg.trim().replace("\"", "\'")+"</font>");
		}else{
			sb.append("验证通过！");
		}
...
public String submitFormCheck2(){
	...
	public String submitForm() throws Exception {
		...
		//Script s = scripts.get(scriptSeq);
		...
		errorMessage = "提交成功，请耐心等待3分钟左右系统会自动执行脚本的，执行结果会通过qq通知，请在脚本执行成功后把MR转给相关QA验证";
...
/*		int taskEnvExecPoint = Integer.parseInt(taskEnvironmentExecutePoint);
	...
	private void validateFileCharacter() throws Exception {
		StringBuffer sb = new StringBuffer();
		for (int i=0; i<scripts.size(); i++) {
			File f = files.get(i);
			String fileName = filesFileName.get(i);
	        ...
	        try {
	           // System.out.println("以行为单位读取文件内容，一次读一整行：");
	            ...
	            //int line = 1;//可以用来确认第几行，但考虑到好像目前这种写法只能确认第一行，故注释不用.
	            // 一次读入一行，直到读入null为文件结束
	            while ((tempString = reader.readLine()) != null) {
	                // 显示行号
	                //System.out.println("line " + line + ": " + tempString);
	            	if(tempString.indexOf("&") != -1) {
	    				sb.append(fileName + " 脚本文件中包含'&'字符,请确认并去除！否则会卡的\r\n");
	    			}
	                //line++;
	            }
	            reader.close();
	            ...
	            //e.printStackTrace();
	        	throw new Exception("校验文件出现异常，请重新提交！");
	        } finally {
	            if (reader != null) {
	                try {
	                    reader.close();
	...
	private void validateSelectedEnvRelation(String environmentsStr) throws Exception {
		...
		for (String envId : envIds) {
			Environment env = environmentAccessService.findById(Long.parseLong(envId.trim()), this.getUserContext());
	...
	private List<ScriptDestinationRelation> assembleScriptDestinationRelations(List<Script> scripts, String destinationsStr) {
		...
		for (int i=0; i<ds.length; i++) {
			ids[i] = Long.parseLong(ds[i].trim());
		}
	...
	private List<ScriptEnvironmentRelation> assembleScriptEnvironmentRelations(List<Script> scripts, String environmentsStr, String envExecutePointsStr) {
		...
		for (int i=0; i<eeps.length; i++) {
			...
			for (String e : es) {
				if (e.equals(envId)) {
					Environment env = environmentAccessService.findById(Long.parseLong(envId.trim()), this.getUserContext());
					DictExecutePoint execPoint = dictExecutePointAccessService.findById(Long.parseLong(execPointId.trim()), this.getUserContext());
