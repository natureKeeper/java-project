...
@Comment(
		"*****************************************"
		+ "\r\n任务：IntegrationMockClassGenerator"
		+ "\r\n用途：生成接口模拟类实现"
		+ "\r\n当IRMS调用其他系统接口时，如果对端系统不可用，可以用这个模拟"
		+ "\r\n*****************************************")
public class IntegrationMockClassGenerator extends GeneralGenerator {
	@SuppressWarnings("unused")
	private static final transient Log log = LogFactory.getLog(IntegrationMockClassGenerator.class);
	...
	public void execute(InformationContainer infoContainer, MessageContainer msgContainer) throws Exception {
		...
		if ("N".equalsIgnoreCase(irmsProvideService)) {
			...
			String integrationMockClassFullName = integrationInterfacePackagePath + ".usersession.Integration" + classUpperName + "Mocker";
			...
			map.put("functionNameInT3Doc", integrationFunctionNameInT3Doc);
			...
			String integrationMockClassFullPath = integrationProjectSrcPath + integrationMockClassFullName.replace(".", File.separator) + ".java";
//			createOrUpdateFile(integrationMockClassFullPath, "template1", "template2", "}", map, msgContainer);
			createFile(integrationMockClassFullPath, "template1", map, msgContainer);
		} else {
			System.out.println("此接口调用方向为其他系统使用IRMS，不需要生成接口模拟类");
		}
	}
