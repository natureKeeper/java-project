...
<#else>
	<#list fieldDefs as e>
key=${e.fieldEnName};
type=${e.javaFieldDataType};
		<#if 'date' == e.javaFieldDataType>
value=<#noparse><#if valueObject.</#noparse>${e.javaFieldName?substring(0,1)?lower_case}${e.javaFieldName?substring(1)}<#noparse>?exists>${valueObject.</#noparse>${e.javaFieldName?substring(0,1)?lower_case}${e.javaFieldName?substring(1)}<#noparse>?string("yyyy-MM-dd HH:mm:ss")}</#if></#noparse>
		<#elseif e.javaFieldDataType?starts_with('enum')>
value=<#noparse><#if valueObject.</#noparse>${e.javaFieldName?substring(0,1)?lower_case}${e.javaFieldName?substring(1)}<#noparse>?exists>${valueObject.</#noparse>${e.javaFieldName?substring(0,1)?lower_case}${e.javaFieldName?substring(1)}<#noparse>.getText()}</#if></#noparse>
		<#else>
value=<#noparse>${valueObject.</#noparse>${e.javaFieldName?substring(0,1)?lower_case}${e.javaFieldName?substring(1)}<#noparse>?if_exists}</#noparse>
		</#if>
##############arguments spliter#################
	</#list>
</#if>