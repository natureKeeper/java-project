...
@Comment(
		"*****************************************"
		+ "\r\n任务：Axis14ServiceInterfaceGenerator"
		+ "\r\n用途：生成axis14接口类的接口"
		+ "\r\n*****************************************")
public class Axis14ServiceInterfaceGenerator extends GeneralGenerator {
	@SuppressWarnings("unused")
	private static final transient Log log = LogFactory.getLog(Axis14ServiceInterfaceGenerator.class);
	...
	public void execute(InformationContainer infoContainer, MessageContainer msgContainer) throws Exception {
		...
		axis14PackagePath = axis14PackagePath + "." + remoteSystemName + ".service";
		...
		String interfaceName = serviceInterfaceClassUpperName.replace("Service", "WebService");
		...
		if ("Y".equals(irmsProvideService)) {
			axis14InterfaceClassFullName = axis14PackagePath + ".Irms" + interfaceName;
		} else {
			axis14InterfaceClassFullName = axis14PackagePath + "." + remoteSystemUpperName + interfaceName;
		}
...
//		for (MessageClassDefinition mcd : msgClazzDefs) {
//			mcd.getFieldInfos().size()
//			for (FieldInfo fi : mcd.getFieldInfos()) {
//				fi.getFieldEnName();
//				fi.getFieldCnName();
//				fi.getFieldDataType();
//				fi.getFieldDescription()
//			}
//		}
